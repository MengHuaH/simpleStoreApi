...
export class PasskeyService {
  ...
  async verifyPasskeyLogin(
    ...
    const isValid = await this.verifyPasskeyAssertion(
      passkeyData,
   ...
   * 验证Passkey断言（简化版）
   */
  private async verifyPasskeyAssertion(
    assertion: any,
    ...
    try {
      ...
      if (await bcrypt.compare(assertion, storedCredential)) {
        return true;
      }
      return false;
    } catch (error) {
  ...
  private async findUserByPhone(
    ...
    switch (subjectType) {
      ...
      case SubjectTypeEnum.CommunityStaff:
        return await this.communityStaffRepository.findOne({
          where: { phone },
        });
      default:
        return null;
